...
disp('travelDist is not supported on Create 2') 
...
try
    ...
    %WaitDist is not supported on Create2. Instead read the Distance sensor
%     if (distance ~=0)
%         
...
while(DistTraveled < distance)
    DistTraveled = DistanceSensorRoomba(serPort) +DistTraveled
    pause(td)
end
SetFwdVelRadiusRoomba(serPort, 0, 0);
pause(td)
